...
| Control-tower UX (`CTL-01`)                        | Implemented locally     | Focused tests Passing              | Five-lens summary SQL, bounded pagination and explicit structured vendor allocation metadata are corrected; focused backend reconciliation and Placement browser projection pass, while the full authored suite remains not run                      |
