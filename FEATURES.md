...
| INT-02 | Conversational operations through chat and WhatsApp            | User-directed conversational operations        | Implemented — provider activation pending | Implemented / Not Run | FND-02, DAT-01, GOV-01, INT-01 |
...
| Operations UX (`OPS-01/02/03`)                     | Implemented locally                       | Implemented / Not Run                               | Every register has compact record cards and accessible action sheets; focused responsive/idempotency cases are authored while deeper concurrency remains planned                                                                                                                                                                                                                   |
| Finance UX (`FIN-01/02/03`)                        | Implemented locally                       | Implemented / Not Run                               | Every register has compact record cards and accessible action sheets with local failure feedback; deeper ledger reconciliation remains planned                                                                                                                                                                                                                                     |
| Control-tower UX (`CTL-01`)                        | Implemented locally                       | Prior focused Passing; latest Implemented / Not Run | Five-lens SQL and vendor metadata are corrected; drill rendering is exact-query keyed; compact mobile actions/filters/KPIs are implemented; portfolio cards now use canonical per-location signals and the prototype's monogram/status/G-Y-R/fill hierarchy. Prior vendor projection passes; the latest responsive/card cases and corrected four-lens drill journey remain not run |
| Responsive application UI (`UI-01`)                | Implemented locally                       | Implemented / Not Run                               | Prototype-derived dark operations-console tokens cover the shared surfaces; authenticated `/` and `/app` resolve to an authorized role home, desktop navigation is domain-grouped with direct Finance links, and mobile exposes three capability-derived primary destinations plus More; new focused checks were not run                                                           |
| Responsive workbench components (`UI-02`)          | Implemented locally                       | Implemented / Not Run                               | Shared tabs/sheets/forms/details/metrics, server-filtered reconciliation, list-first Users, compact Control disclosures/actions/KPIs, and prototype-aligned portfolio cards are implemented; latest focused checks were not run                                                                                                                                                    |
| Conversational operations (`INT-02`)               | Implemented — provider activation pending | Implemented / Not Run                               | Authenticated chat, scoped reads, confirmation-gated business commands, DAT/GOV file handoff, verified WhatsApp text/media and outbound reply/alert queue, consent, idempotency and audit are implemented; provider/scanner activation and release testing remain pending                                                                                                          |
| Reusable demo environment (cross-feature)          | Implemented locally                       | Implemented / Not Run                               | Dataset `2026.09.2` expands the canonical showcase while retaining the prior 12-database-check/4-browser-journey passing evidence; new manifest/reconciliation checks were not run                                                                                                                                                                                                 |
| Jurigari production demo profile (`JURIGARI-DEMO`) | Complete / AWS deployed                   | Focused + production smoke Passing                  | Explicit idempotent/adoptable `JG` profile, two protected Tenant Owners, workbook chain, immutable exact finance reconciliation, and secret-free verifier; unit, PostgreSQL, Chromium, AWS replay/readiness, and both-user HTTPS authentication pass                                                                                                                               |
...
| **INT-02** | `/app/assistant`                                                                                        | Authenticated English threads, scoped reference/report/insight reads, strict confirmation-gated master/operations/finance/approval commands, DAT/GOV files, and verified WhatsApp text/media/replies/consented alerts                                                     |
...
| Messaging          | AWS SES owner-activation delivery plus signed Meta WhatsApp input, provider-host media, transactional replies, consented approved-template alerts and PostgreSQL retry/dead-letter handling are implemented. SES production access and Meta account/number/template activation remain pending | Implemented / provider activation pending    |
| Conversational AI  | Closed deterministic English commands, scoped parameterized reports/insights and safe Hinglish aliases are implemented. Evaluated Hindi/Indic support and any production model provider remain product/provider decisions                                                                     | English implemented / Indic decision pending |
...
## INT-02 — Conversational operations through chat and WhatsApp

**Status:** Implemented — provider activation pending

**Test status:** Implemented / Not Run

**Outcome:** Authorized users can provide registered normal-English data-entry, status, approval, governed-comment, file, and report requests through authenticated chat or a verified WhatsApp identity and receive a typed, reviewable result without giving the parser authority over identity, tenant, permissions, SQL, URLs, or arbitrary APIs. Writes require confirmation; scoped reads execute immediately.

### UX flow and behavior

1. The in-app Assistant resolves the logged-in session, active tenant, membership, roles, scopes, and assurance level on the server. WhatsApp requires a short-lived one-time link code initiated while authenticated and proven by a correctly signed inbound provider message.
2. Each inbound message is persisted and deduplicated before interpretation. The deterministic English extractor or a future configured model adapter may emit only a versioned intent from the closed conversational command registry.
3. Referenced records are searched within the effective tenant and scope. Missing fields or zero/multiple matches produce clarification; the bot never guesses a tenant or inaccessible record.
4. Files are size/type/checksum/magic-byte validated. CSV/XLSX uses DAT-01 preview and confirmation-gated commit; PDF/JPEG/PNG creates a GOV-01 version that remains quarantined until a production malware scanner is configured.
5. The Assistant displays the registered action, normalized fields, risk and expiry. Client/vendor creation, receipts, operations/finance transitions, approval decisions, comments, imports and documents require confirmation. Financial reversals remain unavailable without a dedicated compensating-entry command.
6. Confirmation rechecks current membership, capability, resource scope, target state, and optimistic record version where applicable. Execution uses a server-derived idempotency key inside the same transaction; it never loops through arbitrary HTTP or model-generated SQL/URLs.
7. Success includes the canonical reference/status. Failure returns a safe correction path and correlation ID. Messages, confirmations, executions, denials, attachment transitions, and provider failures retain audit evidence without leaking raw content, phone numbers, secrets, or cross-tenant data.

### WhatsApp and file rules

- Verify the provider signature over the exact raw body before parsing, identity resolution, or media retrieval.
- Bind a verified sender to one active membership/tenant. Multi-tenant ambiguity must be resolved explicitly and never by selecting the first membership.
- High-risk actions require authenticated in-app step-up unless a future approved tenant policy provides equivalent assurance.
- CSV/XLSX creates a structural preview before commit continues in the governed Data Import screen. PDF/JPEG/PNG is staged in quarantine and is not promoted into governed storage while the production malware scanner is not configured.
- Attachment text is data, not an instruction source. WhatsApp media may be downloaded only from the configured provider API, never an arbitrary URL supplied in a message.
- Signed text/media inbound, transactional replies, low/medium-risk reply confirmation, explicit proactive consent, tenant-timezone quiet hours, unsubscribe, approved-template alerts, and PostgreSQL retry/dead-letter evidence are implemented. They remain disabled until the Meta provider is configured.

### Acceptance criteria and tests

- Unknown, revoked, inactive, unsigned, duplicate, or ambiguous identities fail closed without enumeration.
- Unsupported/low-confidence intent and missing/ambiguous data produce clarification and no mutation.
- Parser output cannot choose tenant, actor, capability, SQL, URL, HTTP method, or an unregistered command.
- Proposal execution rechecks tenant membership, registered capability, scope and optimistic version; approval commands additionally enforce role, expiry and maker-checker segregation.
- Duplicate browser messages, provider deliveries, confirmations, and processing retries create exactly one logical business effect.
- Conversation/file data stays tenant-isolated and excluded from ordinary logs, errors, audit payloads, exports, and model telemetry.
- The responsive Assistant exposes accessible loading, upload, clarification, confirmation, cancellation, execution, failure, and retry states.
- Status reports, reference retrieval, deterministic operational insights and proactive WhatsApp alerts are implemented. Evaluated Hindi/Indic input and browser/mobile push remain pending provider/product decisions.

### Master prompt for Codex

> Implement **INT-02 Conversational operations through chat and WhatsApp** from `FEATURES.md` and `specs/INT-02/spec.md`. Build a PostgreSQL-backed channel-neutral state machine, responsive authenticated Assistant, signed WhatsApp text/media adapter, strict English command registry, scoped reference/report/insight reads, typed confirmation-gated master/operations/finance/approval writes, DAT/GOV file handoffs, outbound reply/approved-template alert delivery with consent/quiet-hours/retry/dead-letter, immutable audit evidence, and focused isolation/idempotency/file/browser tests. Keep external providers fail-closed until configured; add no Redis, queue, object store, or separate worker. Treat parser/model and attachment contents as untrusted data.
