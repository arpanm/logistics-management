...
| Responsive application UI (`UI-01`)                | Implemented locally                       | Implemented / Not Run                               | Prototype-derived dark operations-console tokens cover the shared surfaces; authenticated `/` and `/app` now resolve to Control Tower for authorized internal users or the first permitted workspace for restricted roles, and the three former landing shortcuts are capability-filtered under Quick access; new focused checks were not run                                      |
