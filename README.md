...
2. As Tenant Owner, the successful login opens Control Tower by default. Use the **Quick access** menu group for Operations, Users, and Activity & audit; complete any outstanding onboarding at `/app/setup`, then create scoped users and roles under `/app/access/users` and `/app/access/roles`.
