...
| INT-01/SES    | Verify `mukh.bad@gmail.com` in SES `eu-north-1`, approve the least-privilege EC2 send policy, add protected runtime settings, request production access, deploy, and run the explicit real-provider smoke | AWS activation pending   | Identity verification and AWS sandbox/production-access approval; implementation is complete      |
