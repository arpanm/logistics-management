...
const sectionMatches = [
  ...featuresText.matchAll(
    /^## ((?:FND|MST|OPS|DOC|FIN|CTL|ALT|DAT|GOV|INT|CFG)-\d+) — .+$/gm,
  ),
];
const errors = [];
...
for (let index = 0; index < sectionMatches.length; index += 1) {
  ...
  if (!implementation)
    errors.push(`${id}: missing section implementation status`);
  if (!test) errors.push(`${id}: missing section test status`);
  if (row && implementation !== row.implementation) {
    errors.push(
      `${id}: implementation status drift (register='${row.implementation}', section='${implementation}')`,
    );
  }
  if (row && test !== row.test) {
    errors.push(
      `${id}: test status drift (register='${row.test}', section='${test}')`,
    );
  }
  if (
    implementation === "Complete" &&
    test !== "Passing" &&
    test !== "Implemented / Not Run"
  ) {
    errors.push(
      `${id}: Complete implementation requires Passing or Implemented / Not Run test status`,
    );
  }
...
if (register.size !== sectionMatches.length) {
  errors.push(
    `feature count drift (register=${register.size}, sections=${sectionMatches.length})`,
  );
}

for (const id of todoText.match(
  /\b(?:FND|MST|OPS|DOC|FIN|CTL|ALT|DAT|GOV|INT|CFG)-\d+\b/g,
) ?? []) {
  if (!register.has(id))
    errors.push(`TODO.md references unknown feature ${id}`);
}

if (!readmeText.includes("Agents synchronize this summary")) {
  errors.push(
    "README.md is missing the mandatory status synchronization notice",
  );
}
