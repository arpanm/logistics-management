...
This plan covers the opt-in demo-data bootstrap as one coherent, referentially valid business story. Focused execution was explicitly requested and its evidence is recorded below; unimplemented or deeper assertions remain **Planned / Not Run**.
...
| `packages/db/src/demo-seed.integration.test.ts` | Passing — 4/4 | Real `logistics_test`: migration 029, independent Platform Admin, exact replay, password mismatch/rotation audit, dispositions, and Employee/persona linkage       |
...
The focused evidence proves the primary documented demonstration path. Deeper concurrency, direct-ID isolation, exact independent ledger reconciliation, failure injection, reset, accessibility/mobile, and remaining production role-page assertions stay `Planned` or `Implemented` as recorded below.
...
| DEMO-INT-002   | Sequential idempotency                                     | Integration                   | One completed bootstrap                                                          | Run the identical bootstrap again                                                                                                                          | Same IDs/counts/hashes and financial effects; no duplicate audit/outbox/invitation/email/ledger entries; command reports reconciled/no-op                                                               | Passing     | `demo-seed.integration.test.ts`; local and AWS identical replay reported no data changes                                                                        |
...
| DEMO-INT-005   | Membership-to-Employee invariant                           | Integration                   | Demo identities seeded                                                           | Reconcile membership types and Employee links                                                                                                              | Every INTERNAL persona has exactly one Employee; vendor/driver/client external personas have none; home/scope nodes belong to Demo tenant                                                               | Passing     | `demo-seed.integration.test.ts`; focused linkage assertion passed in the 4/4 integration suite                                                                  |
...
| DEMO-INT-010   | Import disposition sample                                  | Integration/API               | Demo import fixture exists                                                       | Preview/commit/replay sample import and query row disposition/report                                                                                       | Declared CREATE/UPDATE-or-UNCHANGED/REJECT examples are visible and reconciled; replay creates no duplicate business effect                                                                             | Passing     | Integration assertion plus local/AWS manifest query returned `CREATE`, `UPDATE`, `REJECT`                                                                       |
| DEMO-AUD-001   | Bootstrap provenance and audit safety                      | Integration/security          | Completed bootstrap                                                              | Inspect bootstrap manifest, audit/outbox/log capture and redaction                                                                                         | Version, anchor, actor/source, record summary, correlation ID, and outcome are traceable once; no plaintext credentials/tokens/bank numbers                                                             | Implemented | Rotation audit, real security timestamps and secret-free payload passed; broader bootstrap/outbox log-capture assertions remain planned                         |
