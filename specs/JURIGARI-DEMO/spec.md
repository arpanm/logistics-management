...
- First execution is atomic. Same version/hash replay changes no business data or credentials. Tenant-code, reserved-email, or version/hash collision fails before partial materialization. A pristine Jurigari tenant reservation can be adopted only with its exact UUID, an exact one-shot confirmation, a matching Jurigari legal/display name, and zero legal-entity/scope/organization/membership children; normally provisioned tenants are rejected.
