...
| UI02-CTL-019    | Drill scope stability     | Unit/Playwright        | POD/Collection/Trip/Payable portfolios     | Select portfolio; refresh and restore URL                           | No prior-scope flash; exact filtered locations remain stable                                                                         | Implemented / Not Run | Unit 3/3 Passing; corrected Playwright case not run after setup-fixture correction               |
| UI02-CTL-020    | Portfolio visual contract | Source contract        | Multi-location portfolio markup and styles | Inspect the portfolio card presentation contract                    | Monogram, full name/meta, named status, canonical location signals, dashed G/Y/R footer and fill/open value remain wired             | Implemented / Not Run | `apps/frontend/components/control/control-tower.test.ts`; browser containment remains Planned    |
| UI02-CTL-021    | Compact command density   | Source contract        | Compact Control Tower markup and styles    | Inspect disclosure, accessible action names and touch-target styles | Mobile filters default collapsed; actions remain named; 44px action/tab/disclosure targets and compact KPI/intro styles remain wired | Implemented / Not Run | `apps/frontend/components/control/control-tower.test.ts`; browser containment remains Planned    |
| UI02-FDB-022    | Mutation feedback intent  | Unit                   | Shared API feedback helper                 | Compare login/preview requests with explicit create/update feedback | HTTP method alone never emits saved success; only an explicit mutation message is eligible, while reads never emit it                | Implemented / Not Run | `apps/frontend/components/api-feedback.test.ts`                                                  |
